...
rule samtools_stats:
    ...
    params:
        extra = "-t "+"DATA/TST500C_manifest.bed",                       # Optional: extra arguments.
...
rule picardHsMetrics:
    input:
        bam = "DNA_bam/{sample}-ready.bam",
        intervals = "DATA/TST500C_manifest.interval_list" ##Create with gatk ..
    output:
...
rule touchBatch:
    input:
        expand("DNA_bam/{sample}-ready.bam", sample = config["samples"])
    output:
...
rule getStatsforMqc:
    input:
        picardDup = "qc/{sample}/{sample}_DuplicationMetrics.txt",
        ...
        batch =  "Results/batchQC_stats_unsorted.csv"
    output:
...
rule sortBatchStats:
    input:
        ...
        batchUnsorted = "Results/batchQC_stats_unsorted.csv",
        batchDone = expand("qc/{sample}/{sample}_batchStats.done", sample = config["samples"])
    output:
        batch =  "Results/batchQC_stats_mqc.json"
    #params:
    #    dir = config["programdir"]["dir"]
    log:
    ...
    singularity:
        "/projects/wp2/nobackup/Twist_Myeloid/Containers/python3.6.0-pysam-xlsxwriter.simg"
    shell:
        "(python3.6 sortBatchStats.py {input.batchUnsorted} {input.SampleSheetUsed} {output.batch}) &> {log}"
